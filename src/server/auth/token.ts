...
export class Token {
  ...
  static #cripta: Config

  static async fromRequest(req: IRequest) {
    const token = this.fromCookie(req) || this.fromHeader(req)
    return token ? await this.parse(req, token) : null
  }
  ...
  static async parse(req: IRequest, token: string) {
    ...
    return parse(await this.cripta(), token, {
      iss: serveHost,
      aud: [host],
      '_': await this.fingerprint(req),
    })
  }

  static async create(req: IRequest, user: any, exp: number = 7200) {
    const time = Math.floor(Date.now() / 1000)
    const host = this.host(req)

    return create(await this.cripta(), user, {
      iss: host,
      aud: [host],
      iat: time,
      exp: time + exp,
