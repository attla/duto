...
import type { Token } from 'cripta/token'
...
export class Authnz<T extends object> {
  ...
  constructor(token: Token, data: T, abilities: string[], roles: string[]) {
  ...
  static fromToken<T extends object>(token: Token): Authnz<T> | null {
    if (!token || !token?.valid) return null
