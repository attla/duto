...
import { getContext, incrementIslandId } from './context'
...
export async function check(
	...
	try {
		const { html } = await renderToStaticMarkup(Component, props, children, undefined);
...
function shouldHydrate(metadata: IslandMeta | undefined) {
...
export async function renderToStaticMarkup(
	...
	const attrs: IslandAttrs = {}
	serializeSignals(ctx, props, attrs, propsMap)
	const islandId = incrementIslandId(ctx)
	attrs['id'] = islandId.toString()
...
function filteredConsoleError(msg: string, ...rest: any[]) {
	if (
		consoleFilterRefs > 0 &&
		!process.env.DUTO_INTERNAL_TEST_DISABLE_CONSOLE_FILTER &&
		typeof msg === 'string'
	) {
